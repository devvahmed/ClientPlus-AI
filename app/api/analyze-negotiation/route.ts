...
export async function POST(req: NextRequest) {
  try {
    ...
    const matched_service = body.matched_service || body.matchedService || 'AI perception & robotics R&D';
    ...
    const systemPrompt = `You are an elite B2B Sales & Negotiation Strategist for WTechX (a PhD-founded robotics R&D company specializing in LiDAR-inertial SLAM, AI perception, multi-sensor fusion, and robotics simulation).
...
Evaluate their reply carefully and provide strategic negotiation guidance for WTechX:
...
3. Write strategy_hint: 1-2 actionable sales strategy tips for WTechX (e.g., offer phased pilot, highlight specific ROI, propose flexible milestones, or schedule a technical deep-dive).
4. Write subject: Professional counter-offer subject line under 60 chars.
5. Write body: Persuasive counter-reply email addressing their exact point (< 150 words, warm executive tone, no placeholder brackets, signed 'The WTechX Team').
...
Return ONLY pure JSON matching this exact structure:
{
  ...
  "strategy_hint": "actionable sales strategy advice for WTechX",
  ...
  "body": "full counter-reply email body text"
    ...
    if (!apiKey) {
      return NextResponse.json({
        ...
        strategy_hint: 'Propose a 30-day Phased Pilot project at 40% cost to demonstrate clear ROI before full deployment.',
    ...
    console.log(`[Groq Negotiation Analysis] Analyzing client reply for ${company_name}...`);
    ...
    if (!groqRes.ok) {
      ...
      return NextResponse.json({
        ...
        strategy_hint: 'Offer a technical alignment call with WTechX robotics engineering lead and flexible pilot milestones.',
        subject: `Re: WTechX & ${company_name} Collaboration Options`,
        body: `Hi team at ${company_name},\n\nThank you for sharing your feedback. We would be happy to adapt our implementation schedule to fit your team's exact requirements for ${matched_service}.\n\nWould you be open to a brief 15-minute call with our engineering lead next week to explore tailored options?\n\nBest regards,\nThe WTechX Team`
      });
    }
    ...
    return NextResponse.json({
      objection_type: parsed.objection_type || 'General Intent',
      detected_intent: parsed.detected_intent || 'Client requested further information on technical & commercial scope.',
      strategy_hint: parsed.strategy_hint || `Highlight WTechX's specialized ${matched_service} capabilities and offer a flexible pilot roadmap.`,
      subject: parsed.subject || `Re: WTechX & ${company_name} Options`,
      body: parsed.body || rawContent,
    });
  } catch (err) {
    console.error('[Analyze Negotiation API] Error:', err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'Failed to analyze negotiation reply' },
      { status: 500 }
    );
  }
}
